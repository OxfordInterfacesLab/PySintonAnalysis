...
def extract_srv(df, plot_option, srh_fit_options):
    ...
    #let us remove some of the data point that are not useful
    ...
    for i in range(5):  # 5 iterations
        ...
        if len(delta_n2_pos) == 0:  
            ...
            print('High SRV or wrong gGradient - J_0 calculated from Seff instead of gradient')  
        ...
        #if after simplyfying the expression (no Grad) it is still empty, then don't run more
        if len(delta_n2_pos) == 0:  
            break;
    ...
    # Suns_1 = np.logspace(np.log10(np.min(Suns_0)*1.01), np.log10(np.max(Suns_0)*0.99), 40)
    Suns_1 =sp.interpolate.interp1d(delta_n,Suns_0, kind='linear',fill_value="extrapolate")(delta_n2)
    
    #get the Delta_n that corresponds to the Suns in vector above
    # delta_n_suns=sp.interpolate.interp1d(Suns_0, delta_n, kind='linear',fill_value="extrapolate")(Suns_1)
    
    #get the Suns number for Delta_n equal to the dopant density
    Suns_dop=sp.interpolate.interp1d(delta_n[~np.isnan(Suns_0)], Suns_0[~np.isnan(Suns_0)],kind='linear')(np.abs(ndop))
    
    #get the iVos as funciton of the Suns (here extracted from Dn)
    # iVoc_1 = sp.interpolate.interp1d(delta_n2, iVoc, kind='linear',fill_value="extrapolate")(delta_n_suns)
    iVoc_1 = iVoc
    
    # find the iVoc for 1 sun using linear fit of the Suns for Dn>doping
    slope, intercept = np.polyfit(np.log10(Suns_0[Suns_0>Suns_dop]), iVoc_0[Suns_0>Suns_dop], 1)
    SunVoc_0=intercept
    slope, intercept = np.polyfit(np.log10(Suns_1[Suns_1>Suns_dop]), iVoc_1[Suns_1>Suns_dop], 1)
    SunVoc_1=intercept
    ...
    if plot_option==1:
        ...
        # Subplot 4
        axs[1, 1].semilogx(Suns_1, iVoc, 'o', [1],SunVoc_1, '*')
