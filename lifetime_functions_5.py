...
def extract_srv(df, plot_option, srh_fit_options):
    ...
    Seff_1 = np.sqrt(Damb * (1/tau_eff2 - 1/t_Rad - 1/t_Aug)) * np.tan((waf_thick/2) * np.sqrt((1/Damb) * (1/tau_eff2 - 1/t_Rad - 1/t_Aug)))
    ...
    SRH_0 = np.max(1 / (1/tau_eff2 - 1/t_Rad - 1/t_Aug)) * np.array([10, 10, 0]) + np.array([0, 0, 0.56])  # Starting point 10x above tsurf
    SRH_lb = np.max(1 / (1/tau_eff2 - 1/t_Rad - 1/t_Aug))*np.array([1, 1, 0]) # Lower boundary
    SRH_ub = np.max(1 / (1/t_Rad + 1/t_Aug))*np.array([1.2, 1.2, 0])+ np.array([0, 0, 1.12])   # Upper boundary
    ...
    for i in range(3):  # 3 iterations
        ...
        tau_surf=1/tau_eff2 - 1/t_Rad - 1/t_Aug - 1 / SRHfunction(SRH_0[0], SRH_0[1], SRH_0[2], delta_n2, nieff, n_0, p_0)
            
        Joe2 = q * np.gradient(nieff**2 * np.sqrt(Damb * (tau_surf)) 
            * np.tan((waf_thick / 2) * np.sqrt((1 / Damb) * (tau_surf))),delta_n2)
        ...
        if len(delta_n2_pos) == 0:  
            errorCode = 1 # Display an error message
            ...
            Joe2 = np.sqrt(Damb * (tau_surf)) * np.tan((waf_thick/2) * np.sqrt((1/Damb) * (tau_surf))) * (q * nieff**2) / (np.abs(ndop) + delta_n2)
        ...
        def ObjectiveFun(x,delta_n2,tau_eff2,t_Rad,t_Aug,nieff,n_0, p_0,ndop,Joe_avg):
            tau_surf=1/tau_eff2 - 1/t_Rad - 1/t_Aug - 1 /(SRHfunction(x[0], x[1], x[2], delta_n2, nieff, n_0, p_0))
            term1 = (Joe_avg * (np.abs(ndop) + delta_n2)) / (q * nieff**2)
            term2 = np.sqrt(Damb * (tau_surf)) * np.tan((waf_thick/2) * np.sqrt((1/Damb) * (tau_surf)))
            return term1 - term2
    
        # Perform the optimization
        result = sp.optimize.least_squares(ObjectiveFun, SRH_0, bounds=(SRH_lb, SRH_ub),loss='cauchy', method='dogbox',
    ...
    tau_surf=1/tau_eff2 - 1/t_Rad - 1/t_Aug - 1 / SRHfunction(SRH_0[0], SRH_0[1], SRH_0[2], delta_n2, nieff, n_0, p_0)
    Seff_2 = np.sqrt(Damb * (tau_surf)) * np.tan((waf_thick/2) * np.sqrt((1/Damb) * (tau_surf)))
    ...
    if plot_option==1:
        ...
        # Subplot 1
        axs[0, 0].loglog(delta_n, tau_eff, 'o', delta_n2, 1/(1/t_Rad + 1/t_Aug), '-')
        axs[0, 0].loglog(delta_n2, 1/(1/t_Rad + 1/t_Aug + (2*Joe_avg*(np.abs(ndop)+delta_n2)/(waf_thick*q*nieff**2)) + 1/tau_SRH), '-')
    ...
    df.loc[6,sample_name]=1/(1/t_Rad + 1/t_Aug)
